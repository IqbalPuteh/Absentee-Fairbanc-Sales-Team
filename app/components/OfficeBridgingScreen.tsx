...
interface OfficeBridgingScreenProps {
  isCheckedIn?: boolean;
}

const OfficeBridgingScreen = ({
  isCheckedIn = false,
}: OfficeBridgingScreenProps) => {
  const router = useRouter();

  const handleCheckIn = () => {
    router.push("/components/OfficeAttendance?mode=checkin");
  };

  const handleCheckOut = () => {
    router.push("/components/OfficeAttendance?mode=checkout");
  };
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="bg-blue-600 p-4 flex-row items-center">
        ...
        <Text className="text-white text-xl font-bold">Office Attendance</Text>
      </View>
      ...
      <ScrollView className="flex-1 p-6">
        <View className="items-center mb-8">
          ...
          <Text className="text-gray-500 mt-2 text-center">
            Select an action to record your office attendance
          </Text>
        </View>
...
export default OfficeBridgingScreen;
